...
// Protocol constants (kept for documentation)
const _PREFIX_55AA = 0x000055aa; // v3.1-3.4
const _PREFIX_6699 = 0x00006699; // v3.5
const _SUFFIX_AA55 = 0x0000aa55;
const _SUFFIX_9966 = 0x00009966;
    ...
    _handleV34(pkt, port, info) {
        ...
        if (port === 6667) { // encrypted replies
            try {
                ...
                decryptedMsg = decipher.update(cleanMsg, 'binary', 'utf8');
        ...
        try {
            ...
            if (result && result.gwId && result.ip) {
                ...
                this.log.warn(`Discovery - Incomplete data from ${info.address}:${port}`);
            }
        } catch (ex) {
            // Try to extract gwId from the message to avoid spamming logs for known devices
            let gwIdMatch = decryptedMsg.match(/"gwId"\s*:\s*"([^"]+)"/);
            if (gwIdMatch && this.discovered.has(gwIdMatch[1])) {
                // Device already discovered, ignore parse errors for subsequent messages
                return;
            }
